...
local function EnsureButtonGrabberDB()
	-- Try to get from Enhanced DB first if available
	if E.db.enhanced and E.db.enhanced.minimap and E.db.enhanced.minimap.buttonGrabber then
		return E.db.enhanced.minimap.buttonGrabber
	end

	-- Fallback to WarcraftEnhanced DB (legacy/backup)
	E.db.warcraftenhanced = E.db.warcraftenhanced or {}
	local db = E.db.warcraftenhanced.buttonGrabber
	if not db then
		-- Trying to find P table but might be different structure
		local defaultBase = P.enhanced and P.enhanced.minimap and P.enhanced.minimap.buttonGrabber
		if not defaultBase and P.warcraftenhanced then defaultBase = P.warcraftenhanced.buttonGrabber end
		
		db = E:CopyTable({}, defaultBase or {})
...
local function UpdateButtonGrabberModule(callback)
	-- Try to get Enhanced module first (most likely active)
	local module = E:GetModule("Enhanced_MinimapButtonGrabber", true)
	
	-- Fallback to WarcraftEnhanced module
	if not module then
		module = E:GetModule("WarcraftEnhanced_MinimapButtonGrabber", true)
	end
	
	if not module then return end

	-- module.db is handled internally by the modules now or via EnsureButtonGrabberDB
							...
							args = {
								...
								mbfControlEnabled = {
									...
									set = function(_, value)
										...
										local MBG = E:GetModule("Enhanced_MinimapButtonGrabber", true)
										if MBG and MBG.HandleEnableState then
											MBG:HandleEnableState()
										end
										
										-- Always try legacy module too just in case
										local WEMBG = E:GetModule("WarcraftEnhanced_MinimapButtonGrabber", true)
										if WEMBG and WEMBG.HandleEnableState then
											WEMBG:HandleEnableState()
										end
