...
function NP:SetSize(frame)
	if InCombatLockdown() then
		...
		if self.db.clickThrough[unitType] then
			...
			local plateSize = self.db.plateSize or {}
			local defaults = P.nameplates and P.nameplates.plateSize or {}

			local width, height
			if unitType == "friendly" then
				width = plateSize.friendlyWidth or plateSize.personalWidth or plateSize.width or defaults.friendlyWidth or defaults.personalWidth or defaults.width or 1
				height = plateSize.friendlyHeight or plateSize.personalHeight or plateSize.height or defaults.friendlyHeight or defaults.personalHeight or defaults.height or 1
			else
				width = plateSize.enemyWidth or plateSize.width or defaults.enemyWidth or defaults.width or 1
				height = plateSize.enemyHeight or plateSize.height or defaults.enemyHeight or defaults.height or 1
			end
