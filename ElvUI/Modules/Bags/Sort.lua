...
function B.Sort(bags, sorter, invertDirection)
	...
	local db = B.db or E.db.bags
	B:BuildBlacklist(db.ignoredItems)
...
function B.FillBags(from, to)
	...
	if #specialtyBags > 0 then
		B:Fill(from, specialtyBags, db.sortInverted)  -- Add reverse parameter
	end

	B.Fill(from, to, db.sortInverted)  -- Add reverse parameter
	wipe(specialtyBags)
end
...
function B.SortBags(...)
	local db = B.db or E.db.bags
	local actualReverse = db.sortInverted  -- Default to setting from options
	local args = {...}
	local startIndex = 1
