...
function AB:StyleButton(button, noBackdrop, useMasque)
	...
	if count then
		...
		count:Show()  -- Explicitly show count (reagents, stacks, charges)
	end
...
function AB:UpdateButtonConfig(bar, buttonName)
	...
	bar.buttonConfig.hideElements.count = false  -- Always show count (reagents, stacks, etc)
...
function AB:LAB_ButtonUpdate(button)
	...
	-- Fix count display - restore size if ButtonFacade set it to 0
	if button.count then
		...
		button.count:Show() -- Ensure it's always visible
