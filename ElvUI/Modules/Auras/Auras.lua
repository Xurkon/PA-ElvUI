...
function A:UpdateTime(elapsed)
	...
	-- Ensure timeColors exists before accessing it (COOLDOWNS DISABLED FIX)
	self.timeColors = self.timeColors or E.TimeColors
	local color = self.timeColors and self.timeColors[id]
	if color then
...
function A:SetAuraTime(button, expiration, duration)
	...
	if not button.timeLeft then
		...
		-- Ensure timeColors is set before OnUpdate runs
		button.timeColors = button.timeColors or E.TimeColors
