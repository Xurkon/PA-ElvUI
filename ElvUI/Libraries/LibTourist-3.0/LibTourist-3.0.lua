...
do
	...
	for continentID, continentName in ipairs(continentNames) do
		...
		local continentData = zones[continentName]
		if continentData then
			continentData.texture = GetMapInfo()
		end
		local zoneNames = { GetMapZones(continentID) }
		local continentYards = continentData and continentData.yards
		for _ = 1, #zoneNames do
			...
			if zones[name] and continentYards then
