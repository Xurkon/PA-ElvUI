...
function UF:UpdateAllHeaders(event)
	...
	-- Always hide Blizzard party and raid frames
...
function ElvUF:DisableBlizzard(unit)
	...
	-- Always hide raid frames
	elseif (unit:match("(raid)%d?$") == "raid") then
	...
	function UF:PLAYER_ENTERING_WORLD()
		if not hasEnteredWorld then
			...
			if CompactRaidFrameManager then
				...
				CompactRaidFrameManager:SetParent(E.HiddenFrame)
				if CompactRaidFrameManager.SetSetting then
					CompactRaidFrameManager:SetSetting('IsShown', '0')
				end
			end
			...
			if CompactPartyFrame then
				...
				CompactPartyFrame:SetParent(E.HiddenFrame)
			end
			...
			if PartyMemberBackground then
				...
				PartyMemberBackground:SetParent(E.HiddenFrame)
			end
		else
...
function UF:Initialize()
	...
	-- Always hide Blizzard party/raid interface options panel button
