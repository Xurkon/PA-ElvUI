...
function FogClear:OnInitialize()
	...
	-- Ensure global errata exists
	if not self.db.global.errata then
		self.db.global.errata = {}
	end
	self.overlays = self.db.global.errata
	
	-- Ensure the wildcard entry exists
...
function FogClear:OnDisable()
	self:UnhookAll()
	-- Reset initialization flags when module is disabled
	self.texturesInitialized = false
	self.updateQueued = false
...
local textureCache = {} -- Cache to avoid repeated texture lookups
local function updateOverlayTextures(frame, frameName, scale, alphaMod)
	...
	-- Ensure overlays table exists for this map
	if not self.overlays[mapFileName] then
		self.overlays[mapFileName] = {}
	end
	local overlayMap = self.overlays[mapFileName]
...
-- Pre-initialize textures in the background to avoid first-open stutter
function FogClear:PreInitializeTextures()
	if self.texturesInitialized then return end
	...
	NUM_WORLDMAP_OVERLAYS = max(NUM_WORLDMAP_OVERLAYS, numTexturesToPreCreate)
